...
const CompanyArticleList: React.FC<CompanyArticleProps> = ({ articles }) => {
  return (
    ...
    {articles?.map((article, index) => (
      <a href={article.url} key={index} target="_blank" rel="noopener noreferrer" className="block mb-8 hover:bg-gray-100 p-4 rounded-lg">
        <div className="flex flex-col md:flex-row">
          <div className="md:w-1/4">
            <img src={article.ogp_image_url} alt={article.title} className="w-full h-auto rounded-md"/>
          </div>
